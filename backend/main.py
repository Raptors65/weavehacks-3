...
from learning import get_similar_successful_fixes, format_similar_fixes
...
async def fix_task(task_id: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            # Run the fix agent with similar fixes context
            logger.info("Running fix agent for task %s", task_id)
            fix_result = await run_fix_agent(repo_path, task_data, similar_fixes_text)
